...
"""
Shadow Detection Algorithm - Arevalo
------------------------------------

This script implements the shadow detection method proposed by V. Arévalo, J. González, and G. Ambrosio.
...
Methodology:
    The algorithm utilizes a region growing approach combined with C3 color component analysis to detect shadows.
    It involves:
    1. Pre-processing: Converting BGR to C1C2C3 and HSV color spaces.
    2. Gradient Calculation: Using Sobel operators to find gradients.
    3. Seed Selection: Identifying potential shadow seeds based on statistical properties (mean and variance) in the C3 channel and intensity.
    4. Region Growing: Expanding from the seeds to cover shadow regions based on similarity criteria.
    5. Post-processing: Morphological operations (fill gap) to refine the shadow mask.
    6. Georeferencing: Preserving the spatial reference of the original image in the output.
...
##            print(k)
    ...
    # Saving output with georeference using rasterio
    import rasterio
    ...
    out_path = os.path.join(folder_path, base_name + '_Arevalo.tif')
    with rasterio.open(out_path, 'w', **profile) as dst:
        dst.write(fermeture_morph, 1)
